...
def create_five_post(post_repo, one_post):
    list_names_in_test_data = ["a", "b", "c", "d", "f"]
...
def test_pagination(create_five_post, client, post_repo):
    assert post_repo.all()
...
def test_sort(create_five_post, client, post_repo):
    assert post_repo.all()
    order_by = "image_url"
    response = client.get(f"post/?limit={len(create_five_post)}&order_by={order_by}")
    assert response.status_code == HTTPStatus.OK
